...
2. De workflow `.github/workflows/docker-publish.yml` bouwt een Docker-image
   ...
   `ghcr.io/<owner>/jehova-app:latest`.
3. Op de NAS draaien drie containers: `jehova-app` (de applicatie),
...
4. `jehova-app` publiceert poort 3000 rechtstreeks op je NAS. Jij wijst je
...
`jehova-app` (of `jehova-db`/`jehova-redis`) probleemloos verwijderen en
...
1. Ga naar je GitHub-profiel → **Packages** → `jehova-app`.
...
Portainer trekt nu de drie images (`jehova-app`, `postgres:16-alpine` en
`redis:7-alpine`) en start alles. `jehova-app` wacht via de
...
staat alles goed. In Portainer moeten `jehova-app`, `jehova-db` en
`jehova-redis` alle drie een groene/gezonde status tonen.

## Daarna: updaten

Elke push naar `main` zet een nieuwe `jehova-app`-image klaar op ghcr.io —
die haal je zelf op wanneer het jou uitkomt:

1. Portainer → **Stacks** → `jehova-app`.
2. **Pull and redeploy** (herbouwt alleen `jehova-app` met de nieuwste image;
