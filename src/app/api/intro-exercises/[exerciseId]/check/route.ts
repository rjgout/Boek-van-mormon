...
// Zelfde soort directe correctheidscheck als /api/kids-exercises/[exerciseId]/check,
// maar dan tegen de introductiecursus-oefeningtabel.
export async function POST(req: NextRequest, { params }: { params: Promise<{ exerciseId: string }> }) {
  ...
  const exercise = await prisma.introExercise.findUnique({ where: { id: exerciseId } });
  if (!exercise) {
